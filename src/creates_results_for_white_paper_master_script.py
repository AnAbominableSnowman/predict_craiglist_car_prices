...
plot_histogram(cars, "price", 4000)


plot_histogram(cars, "odometer", 10000)
cars = replace_rare_and_null_manufacturer(cars, 3, "Other")
...
# non_tf_idf_columns = [col for col in cars.columns if not col.startswith("TF_IDF")]

# # Select the first 10 columns that start with 'TF_IDF'
# tf_idf_columns = [col for col in cars.columns if col.startswith("TF_IDF")][:10]

# # Combine the two selections
# selected_columns = non_tf_idf_columns + tf_idf_columns
...
# generate_profiling_report(
#     cars, "results/data_profile_cleaned_subsampled_to_one_percent.html", 1
# )
