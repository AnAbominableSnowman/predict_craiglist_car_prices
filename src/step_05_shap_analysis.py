...
#     # Assuming this function loads data as before
...
#     cars = cars.sample(80_000, random_state=2018)
#     cars.pop("price")
#     return cars
...
#     # Ensure categorical columns are treated as strings or categories
...
#     # Print the SHAP summary plot as well
...
def load_data(data_path: str, col_subset):
    ...
    if col_subset is not None:
        cars = cars[col_subset]
    cars = cars.sample(2_000, random_state=2018)
...
def plot_shap_dependence_for_categoricals(
    ...
    for cat_col in categorical_columns:
        print(f"Creating SHAP dependence plot for {cat_col}...")
        shap.dependence_plot(cat_col, shap_values.values, X, show=False)
        plt.savefig(os.path.join(output_dir, f"shap_dependence_{cat_col}.png"))
        plt.close()
